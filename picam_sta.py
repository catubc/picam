...
set_blue_light(P, P.imaging_filtered)


#****************** LOAD FRAME TIMES AND ASIGN TO IMAGING FRAMES ***************
set_frame_times(P, P.imaging_filtered_lighton, P.imaging_onoff)
...
if os.path.exists(cluster_stacks_file+'.npy')==False: 
    ...
    for file_ in files:
        ...
        ave_stack_2d = ave_stack.reshape((180,256,256))     #DON'T HARDCODE THIS....
        print ave_stack_2d.shape
        ave_stack = mask_data(ave_stack_2d, P)
        
        if True:
            print "... normalizing masked stacks ..."
            vmax = np.nanmax(ave_stack); vmin = np.nanmin(ave_stack)
            print "original max and min: ", vmax, vmin
            ave_stack = np.divide(ave_stack - vmin, vmax-vmin)
...
if (method == 2) or (method == 0): 
    ...
    for k in range(len(data_out[0])-1):
        ax.scatter(data_out[0][k][0], data_out[0][k][1], data_out[0][k][2], s=100, c=cm.Blues(int(float(k%min_index)/min_index*256)))
        ax.plot([data_out[0][k][0], data_out[0][k+1][0]], [data_out[0][k][1], data_out[0][k+1][1]],[data_out[0][k][2], data_out[0][k+1][2]], linewidth=3, c=cm.Blues(int(float(k%min_index)/min_index*256)))
    
    for k in range(len(data_out[1])-1):
        ax.scatter(data_out[1][k][0], data_out[1][k][1], data_out[1][k][2], s=100, c=cm.Reds(int(float(k%min_index)/min_index*256)))
        ax.plot([data_out[1][k][0], data_out[1][k+1][0]], [data_out[1][k][1], data_out[1][k+1][1]],[data_out[1][k][2], data_out[1][k+1][2]], linewidth=3, c=cm.Reds(int(float(k%min_index)/min_index*256)))


    for k in range(len(data_out[2])-1):
        ax.scatter(data_out[2][k][0], data_out[2][k][1], data_out[2][k][2], s=100, c=cm.Greens(int(float(k%min_index)/min_index*256)))
        ax.plot([data_out[2][k][0], data_out[2][k+1][0]], [data_out[2][k][1], data_out[2][k+1][1]],[data_out[2][k][2], data_out[2][k+1][2]], linewidth=3, c=cm.Greens(int(float(k%min_index)/min_index*256)))
